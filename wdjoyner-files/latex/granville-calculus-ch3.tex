...
sage: S = lambda n: add([(-1)^i*2^(-i) for i in range(n)])
sage: RR(S(1)); RR(S(2)); RR(S(5)); RR(S(10)); RR(S(20))
1.00000000000000
0.500000000000000
0.687500000000000
0.666015625000000
...
\vskip .1in
\noindent
You can see from the \sage example that the limit does indeed seem 
to approach $2/3$.
...
\vskip .2in
Here is a \sage example illustrating
$\lim_{t = \infty}1/t=\lim_{t = -\infty}1/t=0$.
...
\begin{center}
%\vspace{1.0 cm}
\includegraphics[height=5cm,width=3cm]{limit_1-over-x.eps}
\end{center}
\end{minipage}
\caption{The limits $\lim_{x\to 0+} 1/x = +\infty$, 
$\lim_{x\to 0-} 1/x = -\infty$.}
\label{fig:limit_1-over-x}
\end{figure}

\vskip .1in
\noindent
This graph suggests that 
$\lim_{x\to 0+} 1/x = +\infty$ and
$\lim_{x\to 0-} 1/x = -\infty$, as the above \sage computation confirms.
...
\[
    f(x) = \frac{x^2 - 4}{x - 2}.
\]
For $x = 1$, $f(x) = f(1) = 3$. Moreover, if $x$ approaches the limit 
...
\index{continuous in an interval}
...
different non-overlapping intervals. For example,
...
sage: f
Piecewise defined function with 3 parts, [[(-5, -2), -1], [(-2, 3), 3], [(3, 5), 2]]
...
Here is a \sage example illustrating these facts in a special case.
...
limit above. 
%\footnote{We use the command-line version of \sage, as opposed
